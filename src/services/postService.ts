...
export async function getPostId(url: URL): Promise<string> {
  ...
  if ("redirect_result" in bulkRouteDefinitionsResponse.payload.payloads[url.pathname].result) {
    postId =
      bulkRouteDefinitionsResponse.payload.payloads[url.pathname].result.redirect_result.exports.rootView.props.post_id;
  } else {
    postId = bulkRouteDefinitionsResponse.payload.payloads[url.pathname].result.exports.rootView.props.post_id;
  }
  return postId;
}
...
function consolidateRawPost(post: Post): Post {
  ...
  post.sharedPosts.forEach(p => {
    ...
    post.originalHeight = post.originalHeight > p.originalHeight ? post.originalHeight : p.originalHeight;
    post.originalWidth = post.originalWidth > p.originalWidth ? post.originalWidth : p.originalWidth;
  });
